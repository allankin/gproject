	...
	public void onCreate(Bundle savedInstanceState){
		...
		if(intent!=null){
			...
			if(bundle!=null){
				...
				log.info("contactId:"+contactId+",contactName:"+contactName);
		...
		pushMsgSendBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(content.length()>0){
					DataRunnable dr = (DataRunnable)RunnableManager.getRunnable(TxlConstants.BIZID_REQUEST_DATA);
	...
	private class MessageDetailReceiver extends BroadcastReceiver{
		...
		public void onReceive(Context context, Intent intent) {
			if (TxlConstants.ACTION_MESSAGE_RECEIVED.equals(intent.getAction())) {
	            String msgId = intent.getStringExtra(TxlConstants.INTENT_BUNDLE_MSG_ID);
	            int cId = intent.getIntExtra(TxlConstants.INTENT_BUNDLE_CONTACT_ID, -1);
	            log.info("MessageDetailReceiver rece_contactId:"+cId+", contactId:"+contactId+",msgId:"+msgId);
	            if(contactId==cId){
	            	if(msgId!=null){
	            		PushMsg  pm = PushMsgDao.getSingle(me).getPushMsgByMsgId(msgId);
	            		if(pm!=null){
